...
async function getState() {
  const [products, categories, members, orders, items] = await Promise.all([
    sb('/products?select=*&order=id.asc'),
...
async function getPublicCategories() {
  const [categories, products] = await Promise.all([
    sb('/categories?select=*&order=name.asc'),
    sb('/products?select=category,brand&order=category.asc')
  ]);
...
async function upsertState(body) {
  if (Array.isArray(body.products)) {
    const productIds = idsForFilter(body.products.map(p => p.id));
    if (productIds) await sbDelete(`/products?id=not.in.(${productIds})`);
    else await sbDelete('/products?id=not.is.null');
  ...
  if (Array.isArray(body.categories)) {
    const names = body.categories.map(c => `"${String(c.name || '').replaceAll('"', '\\"')}"`).filter(Boolean).join(',');
    if (names) await sbDelete(`/categories?name=not.in.(${encodeURIComponent(names)})`);
    else await sbDelete('/categories?id=not.is.null');
